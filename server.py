...
# Railway (et la plupart des hébergeurs) capturent stdout via un pipe, pas un
...
# les logs Railway — indispensable pour diagnostiquer quoi que ce soit.
try:
...
# ║                 Ex : 12.0 pour masquer les outsiders au-delà de 12 en live  ║
COTE_MAX     = 12.0
# ║                                                                              ║
...
# ║                    Les chutes affichées dans le classement (et la colonne  ║
# ║                    "0MIN" de l'interface) sont ensuite calculées entre ce  ║
# ║                    screen et la cote actuelle.                             ║
# ║                    Défaut : 0 (screen pris au moment même du départ)       ║
H3_SNAPSHOT_MIN = 0
# ║                                                                              ║
...
# ║                    qui garantit que le screen "0min" (au départ) de la     ║
# ║                    course suivante soit pris à l'heure même si personne    ║
# ║                    ne consulte la page entre deux courses. Garder la même  ║
# ║                    valeur que AUTO_ADVANCE_MIN côté index.html. Défaut : 5 ║
AUTO_ADVANCE_MIN_SERVER = 5
# ║                                                                              ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
#
# NOTE — Déploiement Railway :
...
#     le service Railway est actif).
#   - Selon le plan Railway utilisé, le service peut rester actif en
#     permanence (pas de veille automatique comme sur Render free), sauf si
#     le mode "sleep" est activé manuellement dans les paramètres Railway.
#   - Le PORT est fourni par Railway via la variable d'environnement PORT.
...
def _build_state_payload() -> dict:
    ...
    for num, info in odds.items():
        ...
        # NOTE : h3_snapshot est désormais pris au départ même (0min) et non
        # plus 3min avant — les termes H3→H1 et H1→HD deviennent quasi
        # simultanés en pratique, seul H3→Live (poids 0.8, cf. plus bas) et
        # HD→Live restent pertinents au quotidien.
    ...
    # ── Classement des chutes depuis le screen 0min (départ) ────────────────
    # Compare la cote figée au moment du screen (h3_snapshot, pris au
    # départ) à la cote actuelle pour chaque cheval, et trie de la plus
    # grosse chute (% le plus élevé) à la plus petite. Une chute positive =
    # la cote a baissé (cheval davantage joué) ; une valeur négative = la
    # cote est remontée.
            ...
            try:
                ...
                if odds:
                    ...
                    if (not STATE.get("h3_snapshot_taken")
                        ...
                        print(f"  [SNAPSHOT] 0min (départ) pris pour R{r}C{c} à "
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/", "/index.html"):
            ...
            # qui empêche Railway de mettre le service en veille (si le mode
            # sleep est activé). Ne fait
...
if __name__ == "__main__":
    print()
    print("╔══════════════════════════════════════════════════════╗")
    print("║         TBLAUGRANA BOT  —  v14 (Railway)            ║")
    ...
    print(f"║  Screen      : pris au depart (0min), classement des chutes ║")
    print("╚══════════════════════════════════════════════════════╝")
    print()
